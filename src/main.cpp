				...
				if (s != "") 
				{
					...
					if (event == "telemetry") 
					{
						...
						*/
						...
						// State after delay.
						double x_ 		= x0 + ( v * cos(psi0) * delay );
						double y_ 		= y0 + ( v * sin(psi0) * delay );
						double psi_ 	= psi0 - ( v * delta * delay / mpc.Lf );
						double v_ 		= v + a * delay;
						double cte_ 	= cte0 + ( v * sin(epsi0) * delay );
						double epsi_ 	= epsi0 - ( v * atan(coeffs[1]) * delay / mpc.Lf );
						...
						json msgJson;
						// NOTE: Remember to divide by deg2rad(25) before you 
						// send the steering value back.
